...
namespace MetroAts {
    internal class ATC {
        ...
        public static void BeaconPassed(AtsEx.PluginHost.Native.BeaconPassedEventArgs e) {
            switch (e.Type) {
                case 12:
                    if (ORPPattern != SpeedLimit.inf) ORPPattern.Location = MetroAts.state.Location + e.Optional;
                    break;
                case 32:
                    StationStop = true;
            ...
            if (ATCEnable) {
                ...
                if (CurrentSection.CurrentSignalIndex < 9 || CurrentSection.CurrentSignalIndex == 34 || CurrentSection.CurrentSignalIndex >= 49) {
                    ...
                    if (Time - InitializeStartTime < 3000) {
                        ...
                        if (ATC_WarningBell.PlayState == AtsEx.PluginHost.Sound.PlayState.PlayingLoop && !Noset)
                            ATC_WarningBell.Stop();
                        ...
                        if (ORPPattern != SpeedLimit.inf && (Speed > ORPPattern.AtLocation(Location, ORPPatternDec) || Speed < 5)) {
