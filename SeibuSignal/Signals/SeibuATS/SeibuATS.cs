...
namespace MetroAts {
    internal class SeibuATS {
        ...
        public static SpeedPattern ATSPattern = new SpeedPattern(), SignalPattern = new SpeedPattern(), StopPattern = new SpeedPattern(), LimitPattern = new SpeedPattern();
        ...
        public static void Initialize(BveEx.PluginHost.Native.StartedEventArgs e) {
            ...
            StopPattern = new SpeedPattern();
            LimitPattern = new SpeedPattern();
        ...
        public static void BeaconPassed(BveEx.PluginHost.Native.BeaconPassedEventArgs e) {
            switch (e.Type) {//1 2 5 8 20
                ...
                case 2:
                    ...
                    break;
                case 5:
                    if (StopPattern == SpeedPattern.inf) StopPattern = new SpeedLimit(0, MetroAts.state.Location + 590);
                    break;
                case 8:
                    break;
                case 20:
        ...
        public static void Tick(double Location, double Speed, Section nextSection) {
            ...
            if (PointType == 1) {
                if (nextSection.CurrentSignalIndex == 0) {
                    SignalPattern = new SpeedPattern(PointLocation + 200, 0);
                } else if (nextSection.CurrentSignalIndex == 1) {
                    SignalPattern = new SpeedPattern(PointLocation + 200, 30);
                } else if (nextSection.CurrentSignalIndex == 2) {
                    SignalPattern = new SpeedPattern(PointLocation + 200, 65);
                } else if (nextSection.CurrentSignalIndex == 3) {
                    SignalPattern = new SpeedPattern(PointLocation + 200, 95);
                } else if (nextSection.CurrentSignalIndex == 4) {
                    SignalPattern = new SpeedPattern(PointLocation + 200, MaxOver95 ? 115 : 95);
                }
            } else if (PointType == 2) {
                if (nextSection.CurrentSignalIndex == 0) {
                    SignalPattern = new SpeedPattern(PointLocation + 200, 0);
